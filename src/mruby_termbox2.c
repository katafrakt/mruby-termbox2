...
static mrb_value mrb_tb2_shutdown(mrb_state* arb, mrb_value self) {
  tb_shutdown();
  return mrb_nil_value();
}

static mrb_value mrb_tb2_present(mrb_state* arb, mrb_value self) {
  tb_present();
  return mrb_nil_value();
}

static mrb_value mrb_tb2_clear(mrb_state* arb, mrb_value self) {
  tb_clear();
  return mrb_nil_value();
}
...
// This converts modifiers int (constants starting with TB_MOD_) to an array of symbols
// Supported symbols are: :alt, :ctrl, :shift, :motion
static mrb_value mrb_tb2_modifier_to_symbol(mrb_state* mrb, uint8_t mod) {
  ...
  if (mod & TB_MOD_SHIFT) {
    mrb_ary_push(mrb, modifiers, mrb_symbol_value(mrb_intern_lit(mrb, "shift")));
  }
  if (mod & TB_MOD_MOTION) {
    mrb_ary_push(mrb, modifiers, mrb_symbol_value(mrb_intern_lit(mrb, "motion")));
  }
...
static mrb_value mrb_tb2_poll_event(mrb_state* mrb, mrb_value self) {
  ...
  mrb_value event_const = mrb_const_get(mrb, mrb_obj_value(termbox_mod), mrb_intern_lit(mrb, "Event"));
  ...
  mrb_value args[] = {
    mrb_tb2_event_type_to_symbol(mrb, ev.type),
  ...
  mrb_value event = mrb_funcall_argv(mrb, event_const, mrb_intern_lit(mrb, "new"), 8, args);
...
#define DEFINE_FORMAT_CONST(name) mrb_define_const(mrb, format_mod, #name, mrb_fixnum_value(TB_##name))
#define DEFINE_EVENT_FIELD(index, name) fields[index] = mrb_symbol_value(mrb_intern_lit(mrb, #name))
...
void mrb_mruby_termbox2_gem_init(mrb_state* mrb) {
  ...
  mrb_define_const(mrb, format_mod, "TB_256_BLACK", mrb_fixnum_value(TB_256_BLACK));
